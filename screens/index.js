...
import Icon from "react-native-vector-icons/FontAwesome5"; // @ts-ignore
...
const Maps = ({
  ...
  useEffect(() => {
    if (origin) {
      setDefaultOrigin({ ...defaultOrigin,
  ...
  return <View style={[styles.view, mainContainerStyle]}>
      <View style={{
      zIndex: 1000,
      height: inputValue ? "100%" : 50
    }}>
        {showSearchInput && <GooglePlacesAutocomplete placeholder='Search location' minLength={2} autoFocus={false} returnKeyType={"default"} fetchDetails={true} textInputProps={{
        onChangeText: text => setInputValue(text)
      }} onPress={(data, details = null) => getAddressHandle(details)} styles={autoCompleteStyles} query={{
        key: apiKey,
        language: "en"
      }} />}
      </View>
      ...
      longitudeDelta: LONGITUDE_DELTA
    }} ref={setMapRef} onPress={onMapPress} style={styles.map}>
        {markedLocations && markedLocations.map((item, index) => <Marker key={index} coordinate={{
        latitude: item.latitude,
        longitude: item.longitude
      }} title={item?.title} description={item?.description} pinColor={markerColor}>
            {markerImage && <Image source={{
          uri: markerImage
        }} style={[styles.marker, markerImageStyle]} />}
          </Marker>)}

        <Marker key={1} draggable coordinate={{
        latitude: defaultOrigin.latitude,
        longitude: defaultOrigin.longitude
      }} title={defaultOrigin?.title} description={defaultOrigin?.description} pinColor={markerColor || null} onPress={() => setOriginAddress(defaultOrigin, "origin")} onDrag={() => handleOnDrag()} onDragStart={() => handleOnDragStart()} onDragEnd={e => handleDragOrigin(e)}>
          {markerImage && <Image source={{
          uri: markerImage
        }} style={[styles.marker, markerImageStyle]} />}
        </Marker>
        {destination && <Marker key={2} draggable coordinate={destination} title={destination?.title} description={destination?.description} pinColor={markerColor || null} onPress={() => setOriginAddress(destination, "destination")} onDrag={() => handleOnDrag()} onDragStart={() => handleOnDragStart()} onDragEnd={e => handleDragDest(e)}>
            {markerImage && <Image source={{
          uri: markerImage
        }} style={[styles.marker, markerImageStyle]} />}
          </Marker>}
        {destination && <MapViewDirections origin={{
        latitude: defaultOrigin.latitude,
        longitude: defaultOrigin.longitude
      }} waypoints={[{
        latitude: defaultOrigin.latitude,
        ...
        latitude: destination?.latitude,
        longitude: destination?.longitude
      }} apikey={apiKey} strokeWidth={strokeWidth || 4} strokeColor={strokeColor || "hotpink"} optimizeWaypoints={true} onStart={params => handleOnStart(params)} onReady={result => handleOnReady(result)} onError={errorMessage => handleOnError(errorMessage)} resetOnChange={false} />}
      </MapView>
      {enableDirections && <TouchableOpacity style={styles.directionsContainer} onPress={handleDirections}>

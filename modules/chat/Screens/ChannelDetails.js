...
import { SectionList, Button, View, Text, Pressable, StyleSheet } from "react-native";
...
const GroupChatDetails = ({
  ...
  const bootstrap = async () => {
    const res = await pubnub.objects.getChannelMembers({
      channel: params.id
    });
    ...
    return <View key={item.id} style={styles.ListItem}>
        <View style={styles.ProfileContainer}>
          ...
          <View style={styles.Profile}>
            ...
            {state.user._id === params.custom.owner && <Pressable onPress={() => removeMember(item)}>
